<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="听课日期" prop="tingkeriqi">
          <el-input v-model="ruleForm.tingkeriqi" 
              placeholder="听课日期" clearable  :readonly="ro.tingkeriqi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="听课日期" prop="tingkeriqi">
              <el-input v-model="ruleForm.tingkeriqi" 
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="听课结果" prop="tingkejieguo">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.tingkejieguo" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.tingkejieguo" label="听课结果" prop="tingkejieguo">
                    <span v-html="ruleForm.tingkejieguo"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.tingkeriqi = true;
          ...
          if(o=='tingkejieguo'){
            this.ruleForm.tingkejieguo = obj[o];
	    this.ro.tingkejieguo = true;
            continue;
          }
          if(o=='jiaoshigonghao'){
    ...
    info(id) {
      this.$http({
        url: `tingkejieguotongzhi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.tingkejieguo = this.ruleForm.tingkejieguo.replace(reg,'../../../ssmo8x1z/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `tingkejieguotongzhi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.tingkejieguotongzhiCrossAddOrUpdateFlag = false;
                  this.parent.search();
